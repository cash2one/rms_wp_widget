...
function eggrms_node_delete($node){
	...
	watchdog("eggrms_node_delete","delete: node:".json_encode($node->nid)." remote max id:".$EGG_SE_MAX_POST_ID);
	...
	//delete 
		...
		$post_data=array("authdomain"=>$wp_egg_target_domain,"targetdomain"=>$wp_egg_target_domain,"url"=>url('node/'.$node->nid,array('absolute' => TRUE)));
		post_articles($post_data,$post_url);
	}
