...
if(!wp_next_scheduled("upstory_push_article")){
 wp_schedule_event(time(), "hourly", "upstory_push_article" );
 add_action("upstory_push_article", "upstory_push_function");
...
function upstory_push_function(){
error_log("start:");
	
	$UPSTORY_MAX_POST_NUMBER=get_option("UPSTORY_MAX_POST_NUMBER");
	if(empty($UPSTORY_MAX_POST_NUMBER) || $UPSTORY_MAX_POST_NUMBER<1 || $UPSTORY_MAX_POST_NUMBER>20){
		$UPSTORY_MAX_POST_NUMBER=5;
	}
	error_log("upstory_push_function list:");
	global $wpdb;
	...
	if(empty($UPSTORY_SE_MAX_POST_ID)){
		$UPSTORY_SE_MAX_POST_ID=0;
	}
	$CurrentID=$UPSTORY_SE_MAX_POST_ID+1;
	
	error_log("curren ID: ".$CurrentID."  NewestID:".$NewestID);
	//return;
	$Posts=array();
	if($NewestID>0 && $UPSTORY_SE_MAX_POST_ID<$NewestID){

		$count=0;
		for($i=0;$count<$UPSTORY_MAX_POST_NUMBER && $NewestID>=$CurrentID;$i++){
			...
			$a=format_article($p,$wp_upstory_target_domain);
			//array("content"=>htmlspecialchars(trim(strip_tags($p->post_content))),"contenttitle"=>$p->post_title,"article_time"=>strtotime($p->post_date),"post_id"=>$CurrentID,"domain"=>$wp_upstory_target_domain,"url"=>get_permalink($CurrentID));
		...
		$post_data=array("posts"=>json_encode($Posts),"authdomain"=>$wp_upstory_target_domain);
		//error_log($post_data);
...
function push_on_save($post_id){
	...
	$Posts=array();
	if($post_id>$UPSTORY_SE_MAX_POST_ID){ return;}
	$p=get_post($post_id);
	if(!empty($p)){
		array_push($Posts,format_article($p,$wp_upstory_target_domain));
	}
	if(count($Posts)<=0){
		error_log("no article to posts~");
		return;
...
function push_on_delete($postId) { 
   $postdomain=get_option("UPSTORY_DOMAIN");
	...
	$post_data=array("authdomain"=>$wp_upstory_target_domain,"targetdomain"=>$wp_upstory_target_domain,"url"=>get_permalink($postId));
	post_articles($post_data,$post_url);
}

function format_article($p,$wp_upstory_target_domain){
	if(empty($p)){return array();}
	$first_img=catch_first_image($p->post_content);
	$a= array("content"=>htmlspecialchars(trim(strip_tags($p->post_content))),"contenttitle"=>$p->post_title,"article_time"=>strtotime($p->post_date),"post_id"=>$p->ID,"domain"=>$wp_upstory_target_domain,"url"=>get_permalink($p->ID),"thumbnail"=>$first_img);
	return $a;
}
function get_max_post_id($rmsdomain,$targetdomain){
		$UPSTORY_SE_MAX_POST_ID=get_option("UPSTORY_SE_MAX_POST_ID");
		$ch = curl_init();
		...
		if(!empty($ret) && $ret->error_code==0 ){
			if($ret->ret_value>=$UPSTORY_SE_MAX_POST_ID || $ret->ret_value>1){
				$UPSTORY_SE_MAX_POST_ID=$ret->ret_value;
			}
			if($ret->ret_value==-1){
				$UPSTORY_SE_MAX_POST_ID=0;
			}
			update_option("UPSTORY_SE_MAX_POST_ID",$UPSTORY_SE_MAX_POST_ID);
		}
		return $UPSTORY_SE_MAX_POST_ID;


}
function set_max_post_id($mid){
	update_option("UPSTORY_SE_MAX_POST_ID",$mid);
}
